let minD = Math.min(window.screen.height, window.screen.width);
...
$(document).on('mousemove', function(event) {
  if (isMobile)
    return;
  $('.bg-primary-1').each(function() {
    let offset = $(this).offset();
    let posX = event.pageX - offset.left;
    let posY = event.pageY - offset.top;
    $(this).css('background', 'radial-gradient(' + minD + 'px at ' + posX + 'px ' + posY + 'px, var(--alt-2), var(--alt-1)');
  });
});
window.addEventListener('devicemotion', function(event) {
  ...
  $('.bg-primary-1').each(function() {
    ...
    let cX = centerX / 5;
    let cY = centerY / 5;
    let posX = (centerX + accelerationX * cX) - offset.left;
    let posY = (centerY + scrollTop + accelerationY * cY) - offset.top;
    $(this).css('background', 'radial-gradient(' + minD + 'px at ' + posX + 'px ' + posY + 'px, var(--alt-2), var(--alt-1)');
  });
  }, 100);
});